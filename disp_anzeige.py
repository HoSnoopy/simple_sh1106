...
#Dieses Script zeigt Wetterdaten von einer Textdatei und System-Statusmeldungen an. 
...
file = '/opt/autofs/daten/temp/dht_curr.dat' #Wetter-File
filexists=Path(file)
...
while True:
 if filexists.is_file(): 
     ...
     i = os.popen('ifconfig wlan0 |grep 255.255.255.0 |cut -c 10-')
     i = str(i.read())
     ip = (i.rstrip().split(' '))[1] # Extrahiere Lokale WLAN-IP
...
#Ich habe als Fontgrösse 14 festgeleg. Zeilenabstand ist entsprechend 15 Pixel.
#Schribe anschliessend die oben extrahierten Werte ins virtuelle Display
          draw.text((1,1), "Wetter (" + zeit + "Uhr):", font=font, fill=1)
          ...
          draw.text((1,61), ("Lokal ("+ zeit + "Uhr):" ), font=font, fill=1) 
...
#Der Obere Bereich (Zeile 1-60) wird nun angezeigt
     ...
     virtual.set_position((0, 61)) #Wechsle zum unteren Bereich
     time.sleep(10) #Warte erneut 10 Sekunden
          
     virtual.set_position((0, 0)) #Wechsle zum oberen Bereich


 else:
     draw.text((1,1), file , font=font, fill=1)
     draw.text((12,1), "gibt's nicht!" , font=font, fill=1)
 time.sleep (5) #Warte nochmal 5 Sekunden.
